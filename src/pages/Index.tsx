...
import TimeDilationPanel from '@/components/TimeDilationPanel';
  ...
  return (
    <div className="relative min-h-screen overflow-hidden font-body">
      ...
      <ControlsHUD
        ...
        collapsed={controlsCollapsed} setCollapsed={setControlsCollapsed}
        showTimeDilation={showTimeDilation} setShowTimeDilation={setShowTimeDilation}
      />

      {/* Hint for shortcuts */}
      <div className="fixed bottom-4 left-4 z-30 font-body text-[10px] text-muted-foreground/50">
        Press <kbd className="px-1 py-0.5 rounded bg-muted/50 text-foreground/60 border border-border/30">?</kbd> for shortcuts
      </div>
    </div>
  );
}
