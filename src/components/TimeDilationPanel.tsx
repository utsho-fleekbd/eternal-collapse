 ...
 */
export default function TimeDilationPanel({ mass, visible }: TimeDilationPanelProps) {
  const rs = 2 * mass;
    ...
    <TooltipProvider delayDuration={200}>
      <div className="fixed top-4 left-4 z-40 w-72 rounded-2xl border border-border bg-card/85 backdrop-blur-lg overflow-hidden">
        <div className="p-4 pb-2">
          <h3 className="font-display text-lg text-primary">⏱ Time Dilation</h3>
        ...
        <div className="px-4 pb-4">
            ...
            <thead>
              <tr className="text-muted-foreground border-b border-border">
                ...
                <th className="text-right pb-1.5 font-medium">
                  <Tooltip>
                    <TooltipTrigger className="cursor-help underline decoration-dotted">1hr∞ =</TooltipTrigger>
                    <TooltipContent side="bottom" className="max-w-[200px] text-xs">
                      How long 1 hour of distant-observer time takes at this radius (gravitational time dilation).
                    </TooltipContent>
                  </Tooltip>
                </th>
              </tr>
                  ...
                  <tr key={row.label} className="border-b border-border/30 last:border-0">
                    ...
                    <td className="py-1.5 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <span className={isHorizon ? 'text-destructive font-semibold' : 'text-foreground'}>
          ...
          <p className="mt-2 text-[10px] text-muted-foreground leading-tight opacity-70">
            At the event horizon, time stops relative to a distant observer. A clock at 1.01 Rₛ ticks ~10× slower.
          </p>
        </div>
      </div>
    </TooltipProvider>
